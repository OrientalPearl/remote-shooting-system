...
{	
     ...
     *
     */
	public function deveventlog($serial = '')
	{
	 	$page = I('get.page', C('PAGER'));
	 	
	 	if (!$device_mac)
	 	{
	 		$device_mac = I('post.search_devmac');	 		
	 	}
	 
	 	if ($device_mac)
	 	{
	 		$map['device_mac'] = $device_mac;
	 		$count = M('Device_event_log')->where($map)->count();
	 	}
	 	else
	 	{
		 	$query_string = 'select count(*) from `'
		 			.C(DB_NAME).'`.`wrt_device_event_log` where '
		 			.HaffmanEvent::getWblQueryString(); 	
		   	$Model = new Model();	   	
	 	...
	 	if ($count != 0) {
		 	...
		 	if ($device_mac) {
		 		$map['device_mac'] = $device_mac;
		 		$list = M('Device_event_log')->where($map)->limit($limit)->select();
		 	} else {
			 	$query_string = 'select * from `'
			 			.C(DB_NAME).'`.`wrt_device_event_log` where '
			 			.HaffmanEvent::getWblQueryString()
			 			.' order by log_time desc limit '.$limit;
	...
	public function devAddHandle()
	{
		...
		$disk_size = I('post.disk_size');
		$bwlimit = I('post.bwlimit');
	...
	public function devEditHandle()
	{
		...
		if ($bwlimit == $old_dev[0]['bwlimit']) {
			$data = array (
				...
				'disk_size' => $disk_size * 1000,
				//'bwlimit' => $bwlimit,
				//'bwlimit_sync_seq' => $old_dev[0]['bwlimit_sync_seq'] + 1,
				'area' => $area,
				'remark' => $remark
				//'user_name' => $user_name,
				//'user_id' => $user_id,
