...
class CommonWidget extends Controller
{
    ...
    private function show_side_menu()
    {
		...
		$sub_cache = C('admin_sub_menu');
        $icon = C('admin_big_menu_icon');

        foreach ($cache as $cache_key => $cache_each) {
            if (intval($cache_each['weight']) > $weight) {
	    	unset ($cache [$cache_key]);
            }
        }

		foreach ($sub_cache as $cache_key1 => $cache_each1) {
			foreach ($cache_each1 as $cache_key => $cache_each) {
				if (intval($cache_each['weight']) > $weight) {
					unset ($sub_cache[$cache_key1][$cache_key]);
                }
            }
        }
        ...
        foreach ($cache as $url => $name) {
            if ($i == 1) {
                ...
                foreach ($sub_cache as $big_url => $big_name) {
                    if ($big_url == $url)
                        foreach ($big_name as $sub_url => $sub_name) {
                            ...
                            $menu .= '<li class="' . $css . '"><a href="' . U("Admin/" . "$sub_url") . '"><i class="fa fa-angle-double-right"></i>' . $sub_name['name'] . '</a></li>';
                        }
                }
                ...
                foreach ($cache as $big_url => $big_name) {
                    if ($big_url == $url)
                        foreach ($big_name as $sub_url => $sub_name) {
                            $sub_true_url = explode('/', $sub_url);
                            $css = !strcasecmp($sub_true_url [1], strtolower(ACTION_NAME)) ? "active" : "";
                            $menu .= '<li class="' . $css . '"><a href="' . U("Admin/" . "$sub_url") . '">' . $sub_name['name'] . '</a></li>';
                        }
                }
                $menu .= '</ul></li>';
            } else if ($i == $count) {
